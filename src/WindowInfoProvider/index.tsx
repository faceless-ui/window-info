...
class WindowInfoProvider extends Component<Props, IWindowInfoContext> {
  ...
  updateWindowInfo = (): void => {
    ...
    this.setState({
      ...
      '--vh': viewportHeight,
      breakpoints: {
        xs: window.matchMedia(`max-width: ${xs}px`),
        s: window.matchMedia(`max-width: ${s}px`),
        m: window.matchMedia(`max-width: ${m}px`),
        l: window.matchMedia(`max-width: ${l}px`),
        xl: window.matchMedia(`max-width: ${xl}px`),
      },
